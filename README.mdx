...
[ppx_inline_test](https://github.com/janestreet/ppx_inline_test). Just make sure to
include `ppx_expect.evaluator` as a dependency of the test runner. The [Jane Street
tests](https://github.com/janestreet/jane-street-tests) contains a few working examples
using oasis.

[^1]: By ensuring that, for multiline output, the string delimiters lie on their own lines
    (and are not mixed with the test contents), we make it so conflict markers from
    conflicts in test output lie entirely inside the string literal. Then the file still
    builds after the rebase, and the conflict can be resolved just by building the test
    and accepting the corrections.
