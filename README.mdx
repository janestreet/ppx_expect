...
Expect-tests are built on top of the
[inline test](https://github.com/janestreet/ppx_inline_test)
framework, and provide a `let%expect_test` construct that parallels
`let%inline_test`.
...
Configurations from the underlying
[ppx\_inline\_test](https://github.com/janestreet/ppx_inline_test)
framework also affect `let%expect_test`s.
