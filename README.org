...
=Async.Std.Writer.stdout=, =Lwt_io.stdout= or anything else.
...
define no-op hooks that the user can override. =Async.Std= redefines
this module so when =Async.Std= is opened you can write async-aware
expect test.
...
[[https://github.com/janestreet/jane-street-test][Jane Street tests]] contains a few working examples using oasis.
